...
func TestLexerAdditions(t *testing.T) {
	tt(t, func() {
		...
		test("function(a=b){}",
			...
			token.IDENTIFIER, "a", 10,
			token.ASSIGN, "", 11,
			token.IDENTIFIER, "b", 12,
			token.RIGHT_PARENTHESIS, "", 13,
			token.LEFT_BRACE, "", 14,
			token.RIGHT_BRACE, "", 15,
			token.EOF, "", 16,
		)
		...
		test("this.#p.c",
			...
			token.PERIOD, "", 5,
			token.HASH, "", 6,
			token.IDENTIFIER, "p", 7,
			token.PERIOD, "", 8,
			token.IDENTIFIER, "c", 9,
			token.EOF, "", 10,
		)

		test("/^$.*[a-Z]?(a|B)/ig",
			token.SLASH, "", 1,
			token.EXCLUSIVE_OR, "", 2,
			token.IDENTIFIER, "$", 3,
			token.PERIOD, "", 4,
			token.MULTIPLY, "", 5,
			token.LEFT_BRACKET, "", 6,
			token.IDENTIFIER, "a", 7,
			token.MINUS, "", 8,
			token.IDENTIFIER, "Z", 9,
			token.RIGHT_BRACKET, "", 10,
			token.QUESTION_MARK, "", 11,
			token.LEFT_PARENTHESIS, "", 12,
			token.IDENTIFIER, "a", 13,
			token.OR, "", 14,
			token.IDENTIFIER, "B", 15,
			token.RIGHT_PARENTHESIS, "", 16,
			token.SLASH, "", 17,
			token.IDENTIFIER, "ig", 18,
			token.EOF, "", 20,
		)

		test("/t/i",
			token.SLASH, "", 1,
			token.IDENTIFIER, "t", 2,
			token.SLASH, "", 3,
			token.IDENTIFIER, "i", 4,
			token.EOF, "", 5,
		)

		test("#stuff",
			token.HASH, "", 1,
			token.IDENTIFIER, "stuff", 2,
			token.EOF, "", 7,
		)

		test("@thing()",
			token.AT, "", 1,
			token.IDENTIFIER, "thing", 2,
			token.LEFT_PARENTHESIS, "", 7,
			token.RIGHT_PARENTHESIS, "", 8,
			token.EOF, "", 9,
		)

		test("import export default from",
			token.IMPORT, "import", 1,
			token.EXPORT, "export", 8,
			token.DEFAULT, "default", 15,
			token.FROM, "from", 23,
			token.EOF, "", 27,
		)
		...
		test("const {a,...b} = {}",
			...
			token.COMMA, "", 9,
			token.DOTDOTDOT, "", 10,
			token.IDENTIFIER, "b", 13,
			token.RIGHT_BRACE, "", 14,
			token.ASSIGN, "", 16,
			token.LEFT_BRACE, "", 18,
			token.RIGHT_BRACE, "", 19,
			token.EOF, "", 20,
		)

		test("()=>{}",
			token.LEFT_PARENTHESIS, "", 1,
			token.RIGHT_PARENTHESIS, "", 2,
			token.ARROW, "", 3,
			token.LEFT_BRACE, "", 5,
			token.RIGHT_BRACE, "", 6,
			token.EOF, "", 7,
		...
		test("class test extends super",
			token.CLASS, "class", 1,
			token.IDENTIFIER, "test", 7,
			token.EXTENDS, "extends", 12,
			token.SUPER, "super", 20,
			token.EOF, "", 25,
		)
	})
}
